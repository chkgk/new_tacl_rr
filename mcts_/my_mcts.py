...
from mcts_.update_and_push import push_combi_to_interaction
            ...
            if random.uniform(0,1)<epsilon or state.currentRound<=3:
                ...
                for possible_action in range(0, 7):  # [0,1,2..6]
                    demo_state = push_combi_to_interaction(interaction=state.board, rounds=state.currentRound,
                                                           action=possible_action, interaction_id=state.interaction_id,
                                                           this_round_payoff=0)
                    demo_state_embadded = create_embadding_for_qdn(demo_state[demo_state.index <= state.currentRound - 1] ,manual_features_file,True)
    ...
    def __init__(self, state, parent,value_model,manual_features_file,bert_features_file):
        ...
        for possible_action in range(0,7):#[0,1,2..6]
            demo_state = push_combi_to_interaction(interaction=self.state.board, rounds=self.state.currentRound,
                                                              ...
                                                              this_round_payoff=0)
            demo_state_embadded = create_embadding_for_qdn(demo_state[demo_state.index<=self.state.currentRound-1],manual_features_file,True)
...
class mcts():
    ...
    def search(self, initialState,value_model,manual_features_file,bert_features_file):
        self.root = treeNode(initialState, None,value_model,manual_features_file,bert_features_file)
        count=0
        if self.limitType == 'time':
            timeLimit = time.time() + self.timeLimit / 1000
            while time.time() < timeLimit:
                print(f'mcts_is_live!')
                self.executeRound(value_model,manual_features_file,bert_features_file)
                count+=1
        else:
    ...
    def executeRound(self,value_model,manual_features_file,bert_features_file):
        node = self.selectNode(self.root,value_model,manual_features_file,bert_features_file)
        reward, action_in_node = self.rollout(node.state)#,value_model,manual_features_file,bert_features_file)
        #print(reward)
        self.backpropogate(node, reward,action_in_node)

    def selectNode(self, node,value_model,manual_features_file,bert_features_file):
        while not node.isTerminal:
            if node.isFullyExpanded:
                node = self.getBestChild(node, self.explorationConstant)
            else:
                return self.expand(node,value_model,manual_features_file,bert_features_file)
        return node

    def expand(self, node,value_model,manual_features_file,bert_features_file):
        #print(node.state.currentRound)
        actions = node.state.getPossibleActions()
        for action in actions:
            if action not in node.children.keys():
                newNode = treeNode(node.state.takeAction(action), node,value_model,manual_features_file,bert_features_file)
