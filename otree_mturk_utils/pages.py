...
class DecorateIsDisplayMixin(object):
    ...
    def __init__(self):
        ...
        # We need to edit is_displayed() method dynamically, when creating an instance, since custom use is
        # that it is overriden in the last child
...
class CustomMturkWaitPage(WaitPage):
    ...
    # Deault attributes
    use_task = False
    # How much the participant should be payed by second spent on the wait page, and by task done.
    # The result will be stored in participant.vars['payment_for_wait']
    pay_by_task = 0
    pay_by_time = 0
    ...
    # parameters for finish experiment after startwp_timer_to_timeout seconds
    startwp_timer_to_timeout = 600
    ...
    def dispatch(self, *args, **kwargs):
        ...
        print('curparticipant._last_page_timestamp:', curparticipant._last_page_timestamp)

        now = time.time()
        time_passed = now - curparticipant._last_page_timestamp
        print('time_passed:', time_passed)
        if time_passed > self.startwp_timer_to_timeout:
            print('timeout in dispatch:', time_passed)
