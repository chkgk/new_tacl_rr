...
class Introduction(CustomMturkPage):
    ...
    def vars_for_template(self):
        if self.player.id_in_group == 2:  # run this once
...
class ReceiverPage(CustomMturkPage):
    ...
    # if the sender didn't choose his choice after seconds_wait, an option will be chosen randomly
...
class Results(CustomMturkPage):
    ...
    def vars_for_template(self):
        if self.group.receiver_choice:  # if the receiver chose Status quo
            ...
            other_choice = 'Action'
            other_gain_receiver = int(abs(self.group.lottery_result))
        ...
        return {
            ...
            'lottery_result': int(self.group.lottery_result),
            'other_gain_receiver': other_gain_receiver,
            'other_gain_sender': other_gain_sender,
            'receiver_payoff': int(abs(self.group.receiver_payoff)),
            'sender_payoff': self.group.sender_payoff,
            'receiver_negative_result': int(abs(self.group.lottery_result)),
...
class GameOver(CustomMturkPage):
    ...
    """
    template_name = 'first_exp/GameOver.html'
    ...
    def vars_for_template(self):
        ...
        if self.player.id_in_group == 2:  # calculate the receiver final payoff
            ...
            receiver_p_to_bonus = float(receiver_total_points / self.session.vars['max_points'])
            ...
            print('max points for receiver:', self.session.vars['max_points'])
...
class OnePlayerWait(Page):
    ...
    """
    template_name = 'first_exp/OnePlayerWait.html'
